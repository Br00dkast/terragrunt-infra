...
variable "lxc_containers" {
  ...
  type = map(object({
    ...
    dns_config = optional(object({
      ...
      enable_a_record = optional(bool, true),
      
      # Default is 'null' (no CNAME)
      cname_alias     = optional(string, null)
      
    }), {}) # Default for the whole object is an empty map {}
    # --- END OF NEW BLOCK ---
...
variable "shared_domain_suffix" {
  description = "The domain suffix for all services (e.g., 'int.broodkast.eu')."
  type        = string
  default     = "int.broodkast.eu"
}

variable "shared_npm_cname_target" {
  description = "The FQDN of the Nginx Proxy Manager."
  type        = string
  default     = "npm.int.broodkast.eu"
}